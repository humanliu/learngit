...
# 转为列表
rows_data = list(use_kaoqin)# 按行获取数据转换成列表使用
...
n = 2# 初始化循环n
while n < len(list_rows):
    # 初始化各个值
    chuqin = chuqin_x = yingchuqin = chuchai = jiaban = jiaban_x = gongxiu = buxiu = hunjia = sangjia = shijia = shuangxiu = liangxin = jiejia =  0.0
    gou = '√'
    gou_jiaban = '√+'
    c = 'C'
    # ban = re.findall("\d+",)
    ...
    jiediao = '-'
    # 初始化表格颜色
    shuangxiufg = 'FFD9D9D9'#这里要大写
    sanxinfg = 'FFFFFF00'
    gongzuori = '00000000'

    print ('\n',n-1)# 打印序号
    for i in list_rows[n]:
        if i.fill.bgColor.rgb == gongzuori:
            yingchuqin += 1
        # print (i.fill.bgColor.rgb)
        # print('yingchuqin:',yingchuqin,end=" ")
        # 汇总每行的√ 出勤
        if i.value == gou or i.value == jiediao:  
            chuqin += 1.0
            # 根据颜色区分是否为公休日、节假日
    ...
    if shuangxiu+jiejia > 4.5:
        shuangxiu = shuangxiu - (4.5-jiejia) # 剩下拿加班费的双休日
        liangxin = 4.5-jiejia
        buxiu = buxiu + jiaban + shuangxiu  + liangxin 
    else :
        liangxin = shuangxiu
        buxiu = buxiu + jiaban
    ...
    print ('应出勤：',yingchuqin,end='')
    #填表
    table_huizong['AB%d'%(n+3)].value = liangxin
    table_huizong['AA%d'%(n+3)].value = jiejia
    file_kaoqin.save(xxlspath) #保存
    # print('this is ',table_huizong['AA%d'%(n+3)].value)
    # 继续循环
    n += 1
# 测试颜色
